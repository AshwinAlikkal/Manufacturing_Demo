...
ocr_prompt_production = """Extract all handwritten text from this image written in the form of a table. 
Use the following columns only: Date, Production Line, Shift, Machine operating time (hrs), Production Rate (units/hr).
...
STRICTLY IGNORE any fullstops or | in the statements.
"""


ocr_prompt_issues = """Extract all handwritten text from this image written in the form of a table. 
Use the following columns only: Date, Production Line, Shift, Issue Severity Major, Issue Severity Minor, Issue Severity No issues, Comments.
- Date: The date in the format MM/DD/YYYY
- Production Line: The name of the production line (Either it is Line 1 or Line 2 or Line 3)
- Shift: The shift during which the record was made (Either it is Day or Night)

...
def OCR_implementation(uploaded_files):
    ...
    normalize delimiters, and return a clean DataFrame.
    """
    # configure Gemini OCR
    genai.configure(api_key=GEMINI_API_KEY)
    model = genai.GenerativeModel(config.ocr_model)

    # Define expected header and columns
    HEADER = (
    ...
    for uploaded_file in uploaded_files:
        try:
            ...
            resp = model.generate_content([prompts.ocr_prompt, image])
            extracted_text = resp.text.strip()
            logger.info(f"Response generated by Gemini: {extracted_text}")

            # Filter only lines that contain at least one pipe
    ...
    # Parse each row into exactly 9 fields
    parsed_data = []
